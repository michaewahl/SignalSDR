...
import { getCompetitors, getTweddle, getAverageCapabilities } from "@/lib/data";
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Tweddle&apos;s capabilities vs industry average and best-in-class competitors
        </p>
      </div>
        ...
        <CardContent>
          <ResponsiveContainer width="100%" height={350}>
            <BarChart data={barData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
              ...
              <Bar dataKey="Tweddle" fill="oklch(0.795 0.184 86)" radius={[3, 3, 0, 0]} />
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="space-y-3">
          <h2 className="text-sm font-bold text-chart-3">Tweddle Advantages ({advantages.length})</h2>
          {advantages.map((dim) => {
            const diff = tweddle.capabilities[dim] - avgCaps[dim];
            return (
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        ...
                        style={{ color: HEATMAP_COLORS[tweddle.capabilities[dim]] }}
                      >
                        {tweddle.capabilities[dim]}/5
                      </span>
                    </div>
                  </div>
                  <p className="text-xs text-chart-3 mt-1">
                    +{diff.toFixed(1)} above industry average ({avgCaps[dim].toFixed(1)})
                  </p>
                </CardContent>
              </Card>
            );
        ...
        <div className="space-y-3">
          ...
          {gaps.length === 0 ? (
            <Card>
              <CardContent className="pt-4">
                <p className="text-sm text-muted-foreground">
                  Tweddle scores above industry average in all dimensions.
                </p>
              </CardContent>
            </Card>
          ) : (
            gaps.map((dim) => {
              const diff = avgCaps[dim] - tweddle.capabilities[dim];
                ...
                <Card key={dim} className="border-destructive/20">
                  <CardContent className="pt-4">
                    ...
                    <p className="text-xs text-destructive mt-1">
                      -{diff.toFixed(1)} below industry average ({avgCaps[dim].toFixed(1)})
                    </p>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-sm">Who Beats Tweddle (Per Dimension)</CardTitle>
        </CardHeader>
        <CardContent>
              ...
              <div key={dim}>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-xs text-muted-foreground">
                    Tweddle: {tweddle.capabilities[dim]}/5 &middot; {beaters.length} competitors ahead
                  </span>
                </div>
                {beaters.length === 0 ? (
                  <p className="text-xs text-chart-3">No competitor scores higher than Tweddle</p>
                ) : (
