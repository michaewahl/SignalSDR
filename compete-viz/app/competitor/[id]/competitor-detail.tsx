...
import { getCompetitorById, getTweddle } from "@/lib/data";
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-3">
            <h1 className={`text-2xl font-bold ${competitor.is_tweddle ? "text-accent" : ""}`}>
              {competitor.name}
            </h1>
            {competitor.parent_company && (
              <span className="text-sm text-muted-foreground">({competitor.parent_company})</span>
            )}
          </div>
          <p className="text-sm text-muted-foreground mt-1">{competitor.description}</p>
          <div className="flex items-center gap-2 mt-2">
            <Badge style={{ background: CATEGORY_COLORS[competitor.category], color: "white" }} className="text-[10px]">
              {CATEGORY_LABELS[competitor.category]}
            </Badge>
            {!competitor.is_tweddle && (
              <Badge
                variant="outline"
                style={{ borderColor: THREAT_COLORS[competitor.threat_level], color: THREAT_COLORS[competitor.threat_level] }}
              >
      ...
      <div className="grid grid-cols-4 gap-3 text-xs">
        {competitor.founded && (
          <Card><CardContent className="pt-3 pb-3"><p className="text-muted-foreground">Founded</p><p className="font-bold">{competitor.founded}</p></CardContent></Card>
        )}
        {competitor.headquarters && (
          <Card><CardContent className="pt-3 pb-3"><p className="text-muted-foreground">HQ</p><p className="font-bold">{competitor.headquarters}</p></CardContent></Card>
        )}
        {competitor.employees_approx && (
          <Card><CardContent className="pt-3 pb-3"><p className="text-muted-foreground">Employees</p><p className="font-bold">{competitor.employees_approx}</p></CardContent></Card>
        )}
        {competitor.revenue_approx && (
          <Card><CardContent className="pt-3 pb-3"><p className="text-muted-foreground">Revenue</p><p className="font-bold">{competitor.revenue_approx}</p></CardContent></Card>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            <CardTitle className="text-sm">Capability Comparison</CardTitle>
          </CardHeader>
          <CardContent>
      ...
      {!competitor.is_tweddle && (
        <div className="grid grid-cols-2 gap-6">
          <Card className="border-chart-3/20">
            <CardHeader>
              <CardTitle className="text-sm text-chart-3">Tweddle Advantages</CardTitle>
            </CardHeader>
            <CardContent>
              <ul className="space-y-1.5 text-sm">
                {competitor.tweddle_advantages.map((a, i) => (
          ...
          <Card className="border-destructive/20">
            <CardHeader>
              <CardTitle className="text-sm text-destructive">Tweddle Gaps</CardTitle>
            </CardHeader>
            <CardContent>
              <ul className="space-y-1.5 text-sm">
                {competitor.tweddle_gaps.map((g, i) => (
                  <li key={i} className="flex items-start gap-2">
